...
func TestFewerEmbeddingsThanInputsIsAFailure(t *testing.T) {
	...
	got, err := embedAgainst(t, `{"embeddings":[{"values":[0.1,0.2]}]}`, "one", "two")
...
func TestOneEmbeddingPerInputIsTheHappyPath(t *testing.T) {
	...
	got, err := embedAgainst(t, `{"embeddings":[{"values":[0.1,0.2]},{"values":[0.3,0.4]}]}`, "one", "two")
